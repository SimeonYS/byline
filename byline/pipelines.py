...
class BylinePipeline:

    # Database setup
    conn = sqlite3.connect('byline.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `byline`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM byline WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO byline (date, title, link, content)"
